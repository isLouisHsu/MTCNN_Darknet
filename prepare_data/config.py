...
configer.pAnno       = ['../data/12x12.txt', 
                        ...
                        '../data/12x12_test.txt']
configer.pNums       = [10, 10, 15, 15, 30]   # 分别表示每张图片先采样满足个数的负样本(与框个数有关)，在框附近进行采样的`neg`, `part`, `pos`, `landmark`样本个数

configer.rDets       = '../data/rDets.npy'  # 利用PNet生成的候选框
configer.rImage      = '../data/24x24/'
configer.rAnno       = ['../data/24x24.txt', 
                        '../data/24x24_train.txt', 
                        '../data/24x24_valid.txt', 
                        '../data/24x24_test.txt']
configer.rNums       = [20, -1, -1, 80]   # 分别表示每张图片`neg`, `part`, `pos`, `landmark`样本个数，由PNet结果决定

configer.oDets       = '../data/oDets.npy'  # 利用PNet生成的候选框
