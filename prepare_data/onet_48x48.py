...
# 创建文件夹保存截取图片
if not os.path.exists(configer.oImage):
...
SAVE_IMAGE_NAME = configer.oImage + '{:d}.jpg'
SAVE_ANNO_FP    = open(configer.oAnno[0], 'w')
...
# 利用PNet生成候选框
if not os.path.exists(configer.oDets):  # 若已生成则跳过
    ...
    np.save(configer.oDets, DETS_BY_PRNET)

else:
    DETS_BY_PRNET = np.load(configer.oDets, allow_pickle=True)

# 进行采样 
n_image = len(DETS_BY_PRNET)
for i_image, (imname, boxpreds, boxgts, landgts) in enumerate(DETS_BY_PRNET):
    ...
    if landgts.shape[0] == 0:             # 来自`WIDER FACE`，无关键点
        ...
        for i_det, boxr in enumerate(boxpreds):
            ...
            if iour.max() < configer.iouThresh[0]:                          # neg
                ...
                if i_neg < configer.oNums[0]:
                    # 图像
                    imr = image[y1r: y2r, x1r: x2r]
                    imr = cv2.resize(imr, (48, 48))
                    ...
                    print('IMAGE: [{}]/[{}] | DET: [{}]/[{}] | NEG: [{}]/[{}]'.\
                            format(i_image, n_image, i_det, boxpreds.shape[0], i_neg, configer.oNums[0]))
        ...
        FACE_CNT += 1
        i_landmark = 0
        while i_landmark < configer.oNums[-1]:
            ...
            print("IMAGE: [{}]/[{}] | LANDMARK: [{}]/[{}]".\
                format(i_image, n_image, i_landmark, configer.oNums[-1]))
...
with open(configer.oAnno[0], 'r') as f:
...
with open(configer.oAnno[1], 'w') as f:
    f.writelines(TRAIN)
with open(configer.oAnno[2], 'w') as f:
    f.writelines(VALID)
with open(configer.oAnno[3], 'w') as f:
    f.writelines(TEST )